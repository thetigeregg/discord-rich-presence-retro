        ...
        try:
            if alert["type"] == "playing" and "PlaySessionStateNotification" in alert:
                ...
                if (
                    ...
                    self.logger.debug(
                        "Library '%s' is not whitelisted, ignoring", libraryName
                    )
                    return
                if (
                    self.lastSessionKey == sessionKey
                    and self.lastRatingKey == ratingKey
                ):
                    if self.updateTimeoutTimer:
                        self.updateTimeoutTimer.cancel()
                    ...
                    if (
                        ...
                        and self.ignoreCount < self.maximumIgnores
                    ):
                ...
                if self.isServerOwner:
                    self.logger.debug(
                        "Searching sessions for session key %s", sessionKey
                    )
                ...
                if mediaType in ["movie", "episode"]:
                    stateStrings: list[str] = (
                        []
                        if config["display"]["hideTotalTime"]
                        else [formatSeconds(item.duration / 1000)]
                    )
                    if mediaType == "movie":
                        title = f"{item.title} ({item.year})"
                        genres: list[Genre] = item.genres[:3]
                        stateStrings.append(
                            f"{', '.join(genre.tag for genre in genres)}"
                        )
                    ...
                    if state != "playing":
                        if config["display"]["useRemainingTime"]:
                            stateStrings.append(
                                f"{formatSeconds((item.duration - viewOffset) / 1000, ':')} left"
                            )
                        else:
                            stateStrings.append(
                    ...
                    stateText = " · ".join(
                        stateString for stateString in stateStrings if stateString
                    )
                ...
                if config["display"]["buttons"]:
                    ...
                    if mediaType == "movie":
                        ...
                        guidTags = self.server.fetchItem(
                                ...
                                if button["url"] == "dynamic:imdb":
                                    ...
                                    for guid in guids:
                                        if guid.startswith("tmdb://"):
                                            ...
                                            newUrl = guid.replace(
                                                "tmdb://",
                                                f"https://www.themoviedb.org/{tmdbPathSegment}/",
                                            )
                                            break
                                if newUrl:
                                    buttons.append(
                                        {"label": button["label"], "url": newUrl}
                                    )
                        else:
                ...
                if state == "playing":
                    ...
                    if config["display"]["useRemainingTime"]:
                        activity["timestamps"] = {
                            "end": round(
                                currentTimestamp + ((item.duration - viewOffset) / 1000)
                            )
                        }
                    else:
                        activity["timestamps"] = {
            ...
            self.logger.exception(
                "An unexpected error occured in the Plex alert handler"
            )
